...
module.exports = {
  ...
  async execute(interaction, isButton) {
    ...
    try {
      if (!isButton) {
        interaction.reply({ embeds: [titleEmbed(client, "colorBG", "success", "Current song was skipped")], ephemeral: true })
          .catch(err => require('../../modules/handleError')(interaction, err))
      }else{
      ...
      const msg = await MusicPlayerCn.send({ embeds: [titleEmbed(client, "colorBG", "success", "Current song was skipped")] })
        .catch(err => require('../../modules/handleError')(interaction, err))
