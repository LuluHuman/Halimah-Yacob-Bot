    ...
    async execute(interaction) {
        ...
        const embed = new EmbedBuilder()
            ...
            .addFields(
                ...
                {
                    name: "> Members",
                    value: `Totals: ${interaction.guild.memberCount}\nUsers: ${interaction.guild.members.cache.filter(
                        (m) => !m.user.bot
                    ).size
                        }\nBot: ${interaction.guild.members.cache.filter(
                            (m) => m.user.bot
                        ).size
                        }`,
                    inline: true,
                ...
                {
                    name: "> Channels",
                    value: `Category: ${interaction.guild.channels.cache.filter(
                        (channel) =>
                            channel.type == ChannelType.GuildCategory
                    ).size
                        }\nText Channel: ${interaction.guild.channels.cache.filter(
                            (channel) => channel.type == ChannelType.GuildText
                        ).size
                        }\nVoice Channel: ${interaction.guild.channels.cache.filter(
                            (channel) => channel.type == ChannelType.GuildVoice
                        ).size
                        }\nStage: ${interaction.guild.channels.cache.filter(
                            (channel) =>
